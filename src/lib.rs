...
mod tests {
    ...
    // Add any other silly differences here.
    #[test]
...
pub fn tf_plugins_ok(path: std::path::PathBuf) -> bool {
    if let Ok(plugins) = tf_plugins_installed(path) {
        ...
        for x in &all {
            if plugins.iter().any(|(i, _)| i.clone() == *x) {
                found += 1;
            }
        }
...
impl CacheEntry {
        ...
        match &self.source {
            ...
            _ => {
                if let Some(validate) = self.sha256.clone() {
                    let mut fname = std::path::PathBuf::from(&self.cachedir);
    ...
    pub fn cache(&mut self) -> anyhow::Result<std::path::PathBuf> {
        ...
        let check = self
            .check()
            .context("cache file check() failed, refusing to continue")?;
        ...
        match &self.source {
            ...
            CacheSource::CompressedFile { file, dir } => {
                ...
                let mut outdir = std::path::PathBuf::from(&self.cachedir);
                outdir.push(dir);
